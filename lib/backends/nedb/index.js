...
var Datastore = module.exports = function Datastore (options, logger) {
	...
	fs.exists(fullPath,function(exists) {
		...
		fs.readdir( path.resolve(fullPath), function(err, files) {
			...
			if (value.match(/\.db$/g) !== null ) {
			    ...
			    that.pDatabase[collection] = new NeDB({
		    ...
		    if (Object.keys(that.pDatabase).length === 0) {
	...
	Object.keys(that.pDatabase).forEach(function(value, index, array) {
	    that.log.info({ index: index,
			    file: value,
			    length: array.length
			  }, "Loading Database");
	    that.pDatabase[value].loadDatabase();
...
Datastore.prototype.collectionExists = function(type, id, callback) {
    if (typeof this.pDatabase[type + "_" + id ] !== "undefined") {
...
Datastore.prototype.insert = function(type, user, collection, referrer, data) {
	...
	this.pDatabase[type + "_" + user].insert(data, function(err, result) {
